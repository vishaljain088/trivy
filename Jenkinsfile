pipeline {
  ...
  stages {
    ...
    stage('Scan') {
      steps {
	      script {
                      sh 'docker run ghcr.io/aquasecurity/trivy:latest image -f json vishaljain088/dockerwebapp > trivyreport.json | jq '.Results[].Vulnerabilities[].VulnerabilityID' trivyreport.json'
	      }
      }
    }
